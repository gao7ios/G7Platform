 ...
 * @param target The target that was attached to the G7Actions instance.
 * @param indexPath The index path of the object.
 */
typedef BOOL (^G7ActionBlock)(id object, id target, NSIndexPath* indexPath);

/**
 * The attachable types of actions for G7Action.
 */
typedef NS_OPTIONS(NSUInteger, G7ActionType) {
	G7ActionTypeNone      = 0,
	G7ActionTypeTap       = 1 << 0,
	G7ActionTypeDetail    = 1 << 1,
	G7ActionTypeNavigate  = 1 << 2,
};


/**
 * The G7Actions class provides a generic interface for attaching actions to objects.
 *
 * G7Actions are used to implement user interaction in UITableViews and UICollectionViews via the
 * corresponding classes (NITableViewActions and NICollectionViewActions) in the respective
 * feature. G7Actions separates the necessity
 *
...
- (id)attachToObject:(id<NSObject>)object tapBlock:(G7ActionBlock)action;
- (id)attachToObject:(id<NSObject>)object detailBlock:(G7ActionBlock)action;
- (id)attachToObject:(id<NSObject>)object navigationBlock:(G7ActionBlock)action;
...
- (void)attachToClass:(Class)aClass tapBlock:(G7ActionBlock)action;
- (void)attachToClass:(Class)aClass detailBlock:(G7ActionBlock)action;
- (void)attachToClass:(Class)aClass navigationBlock:(G7ActionBlock)action;
...
- (G7ActionType)attachedActionTypesForObject:(id<NSObject>)object;
...
extern "C" {
	 ...
	 */
	G7ActionBlock G7PushControllerAction(Class controllerClass);
...
@protocol G7ActionsDataSource <NSObject>
