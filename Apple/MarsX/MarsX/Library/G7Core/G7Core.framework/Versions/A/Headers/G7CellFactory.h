 ...
 * conforms to the G7CellObject protocol. The created cell should ideally conform to
 * the G7Cell protocol. If it does, the object will be passed to it via
 * @link G7Cell::shouldUpdateCellWithObject: shouldUpdateCellWithObject:@endlink before the
 * factory method returns.
 *
 * This factory is designed to be used with G7TableViewModels, though one could easily use
 * it with other table view data source implementations simply by providing nil for the table
 * view model.
 *
 * If you instantiate an G7CellFactory then you can provide explicit mappings from objects
 * to cells. This is helpful if the effort required to implement the G7Cell protocol on
 * an object outweighs the benefit of using the factory, i.e. when you want to map
 * simple types such as NSString to cells.
 *
 */
@interface G7CellFactory : NSObject <G7TableViewModelDelegate>

/**
 * Creates a cell from a given object if and only if the object conforms to the G7CellObject
 * protocol.
 *
 * This method signature matches the G7TableViewModelDelegate method so that you can
 * set this factory as the model's delegate:
 *
 * @code
 // Must cast to id to avoid compiler warnings.
 _model.delegate = (id)[G7CellFactory class];
 * @endcode
 *
 ...
 */
+ (UITableViewCell *)tableViewModel:(G7TableViewModel *)tableViewModel cellForTableView:(UITableView *)tableView atIndexPath:(NSIndexPath *)indexPath withObject:(id)object;

/**
 * Map an object's class to a cell's class.
 *
 * If an object implements the G7Cell protocol AND is found in this factory
 ...
 *
 * Uses the heightForObject:atIndexPath:tableView: selector from the G7Cell protocol to ask the
 ...
 * cellClass from the G7CellObject protocol.
 *
 ...
 */
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath model:(G7TableViewModel *)model;
 ...
 * create an instance of G7CellFactory.
 *
 * If the cell returns a height of zero then tableView.rowHeight will be used.
 *
 * Example implementation:
 *
 @code
 - (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
 return [G7CellFactory tableView:tableView heightForRowAtIndexPath:indexPath model:self.model];
 }
 @endcode
 *
 * @param tableView The table view within which the cell exists.
...
/**
 * The protocol for an object that can be used in the G7CellFactory.
 *
 * @ingroup TableCellFactory
 */
@protocol G7CellObject <NSObject>
@required
...
/**
 * The protocol for an object that can be used in the G7CellFactory with Interface Builder nibs.
 *
 * @ingroup TableCellFactory
 */
@protocol G7NibCellObject <NSObject>
@required
...
/**
 * The protocol for a cell created in the G7CellFactory.
 *
 * Cells that implement this protocol are given the object that implemented the G7CellObject
 * protocol and returned this cell's class name in @link G7CellObject::cellClass cellClass@endlink.
 *
 * @ingroup TableCellFactory
 */
@protocol G7Cell <NSObject>
@required
/**
 ...
 *
 ...
 id object = [(G7TableViewModel *)tableView.dataSource objectAtIndexPath:indexPath];
 ...
 * @link G7CellFactory::tableView:heightForRowAtIndexPath:model: tableView:heightForRowAtIndexPath:model:@endlink
 * methods on G7CellFactory to achieve the same result. Using the above example allows you to
...
/**
 * A light-weight implementation of the G7CellObject protocol.
 *
 ...
 * to the cell view, you can create an G7CellObject and pass the class name of the cell.
 *
 @code
 [tableContents addObject:[G7CellObject objectWithCellClass:[LoadMoreCell class]]];
 @endcode
 */
@interface G7CellObject : G7Object <G7CellObject>
...
@property (nonatomic, weak) G7Actions *actions;
